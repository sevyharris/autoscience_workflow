...
def run_TS_shell_calc(reaction_index):
    ...
    shell_opt_log = os.path.join(shell_dir, 'fwd_ts_0000.log')
    ...
    if len(reaction.ts['forward']) > 1:
        raise ValueError('multiple forward ts')
    ...
    for i, ts in enumerate(reaction.ts['forward']):
        ...
        os.chdir(start_dir)
        shell_job.wait_all(check_interval=600)

        # TODO restart optimization if it fails. for now, move on


def run_TS_overall_calc(reaction_index):
    ...
    overall_ts_log = os.path.join(overall_dir, 'fwd_ts_0000.log')
    ...
    try:
        with open(shell_opt, 'r') as f:
            ...
            reaction.ts['forward'][0]._ase_molecule = atoms
    except IndexError:
        # handle case where all degrees of freedom were frozen in the shell calculation
        if len(reaction.ts['forward'][0]._ase_molecule) > 3:
